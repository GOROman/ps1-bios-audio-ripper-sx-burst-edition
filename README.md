...
- `make v2-lzma-test` で生成データの圧縮・復元・CRC回帰テストを実行できる
- `make v2-lzma-encode ARGS='--input BIOS --output CONTAINER'` でMacホスト上の候補コンテナを生成できる

実BIOSでのホスト基準値は **197,887 bytes**（524,288 bytesから326,401 bytes削減、62.26%削減）だった。これは200,000 bytes目標を2,113 bytes下回る。現時点ではPS1実行時のLZMA2エンコーダとブラウザ側LZMA2復号が未実装のため、既定のV1送信経路は維持し、V2受信結果は `PENDING` と表示する。ホスト圧縮、エミュレータ動作、実機アナログ音声復元は別々の検証層として扱う。
