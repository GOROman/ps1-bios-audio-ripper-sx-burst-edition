...
const stereoFrames=raw.length/4,left=new Float32Array(stereoFrames+8),right=new Float32Array(stereoFrames+8);
for(let i=0;i<stereoFrames;i++){left[i]=raw.readInt16LE(i*4)/32768;right[i]=raw.readInt16LE(i*4+2)/32768}
...
assert.equal(messages.filter(message=>message.type==='ofdm-packet').length,0,'disarmed worker decoded OFDM before arm');
...
const monoRaw=fs.readFileSync('/tmp/ps1sx-ofdm-mono.raw'),monoFrames=monoRaw.length/4,
    monoLeft=new Float32Array(monoFrames+8),monoRight=new Float32Array(monoFrames+8);
for(let i=0;i<monoFrames;i++){monoLeft[i]=monoRaw.readInt16LE(i*4)/32768;monoRight[i]=monoRaw.readInt16LE(i*4+2)/32768}
