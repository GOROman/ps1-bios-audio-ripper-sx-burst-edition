// Exercise the worker's RS(32+4) erasure decoder against the transmitter's
...
assert.equal(PAYLOAD,144,'PAYLOAD mismatch');

// GF(256) sanity: 1/x * x == 1
for(let x=1;x<256;x++)assert.equal(gfMul(gfInv(x),x),1,`gfInv(${x}) wrong`);

// Transmitter parity: parity[r][i] = XOR_c A(r,c) * data[c][i], A(r,c)=1/(r^(4+c))
function encodeParity(data,k){
  const parity=[];
  for(let r=0;r<4;r++){
    const row=new Uint8Array(PAYLOAD);
    for(let c=0;c<k;c++){const coef=gfInv(r^(4+c));for(let i=0;i<PAYLOAD;i++)row[i]^=gfMul(coef,data[c][i])}
    parity.push(row);
  }
  return parity;
}
...
function trial(k,dropData,dropParity){
  ...
  for(let p=0;p<4;p++)if(!dropParity.includes(p))group.parity.set(p,Uint8Array.from(parity[p]));
  const ok=rsRecover(group,0);
  const enough=(4-dropParity.length)>=dropData.length;
...
// full group, 1..4 data shards lost, exactly enough parity
trial(32,[3],[0]);
trial(32,[0,31],[1,2]);
trial(32,[2,17,29],[0,1,3]);
trial(32,[1,9,20,31],[0,1,2,3]);
...
trial(32,[0,15,31],[0,1]);
trial(32,[24],[]);
// no loss -> trivially ok
trial(32,[],[]);

console.log('PASS ofdm_rs (erasure decode 1..4 shards, GF(256) inverse table)');
