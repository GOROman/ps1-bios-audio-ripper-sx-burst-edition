const RATE=44100,FFT=512,CP=64,SYMBOL=FFT+CP,PACKET_SAMPLES=SYMBOL*5,MONO_PACKET_SAMPLES=SYMBOL*9;
const FIRST_BIN=24,CARRIERS=96,PILOTS=new Set([0,13,27,41,55,69,83,95]),PAYLOAD=144,DATA_SHARDS=32,PARITY_SHARDS=4,GROUP_SHARDS=DATA_SHARDS+PARITY_SHARDS,MAGIC=0x314f5853;
const LEVELS=[-1/Math.sqrt(2),1/Math.sqrt(2)];
...
let W=null;
const PKT_STRIDE=240; /* sizeof(sxd_packet_out_t) */
(async()=>{
  try{
    const res=await fetch(new URL('ofdm-decoder.wasm?v=10',self.location.href));
...
function drainWasm(count){
  ...
  for(let k=0;k<count;k++){
    ...
    const data=new Uint8Array(buf.slice(o,o+176));
    const valid=dv.getInt32(176,true)!==0;
    ...
    const score=dv.getFloat32(224,true),evm=dv.getFloat32(228,true),snr=dv.getFloat32(232,true),
      timing=dv.getFloat32(236,true),swap=dv.getInt32(220,true)!==0,cfo=0,phase=0;
    const packetIndex=dv.getUint32(180,true);lastPacketSeen=Math.max(lastPacketSeen,packetIndex);
    ...
    const group=dv.getUint32(184,true),shard=dv.getInt32(208,true),dataShards=dv.getInt32(212,true),
      total=dv.getUint32(188,true),offset=dv.getUint32(192,true),size=dv.getInt32(216,true);
    totalSize=total;imageCrc=dv.getUint32(196,true);
...
let audioL=[],audioR=[],expectedPacket=0,crcOk=0,crcBad=0,drops=0,started=0,totalSize=0,imageCrc=0,reportAt=0,receivedSamples=0,carrierReported=false,locked=false,lastPacketSeen=-1;
...
if(typeof Worker!=='undefined')for(let index=0;index<FEC_WORKERS;index++){
  try{
    const worker=new Worker('fec-worker.js?v=1');fecPool[index]=worker;
 ...
 * transmitter in src/ofdm_packet.c.  Parity row r (0..3), data column c (0..k-1)
 * uses the Cauchy coefficient A(r,c) = 1 / (r XOR (4 + c)).  With e data shards
...
function rsRecover(group,g){
  ...
  for(let j=0;j<e;j++){
    ...
    for(let c=0;c<k;c++){const sh=group.shards.get(c);if(!sh)continue;const coef=gfInv(r^(4+c));for(let i=0;i<L;i++)s[i]^=gfMul(coef,sh[i])}
    const row=new Uint8Array(e);
    for(let t=0;t<e;t++)row[t]=gfInv(r^(4+missing[t]));
    B.push(row);S.push(s);
  }
...
function nearest(value){return value>=0?1:0}
...
function decode(at,index,score,swap=false,quiet=false){const nibbles=new Uint8Array(352),sources=swap?[audioR,audioL]:[audioL,audioR],channels=sources.map(source=>estimateChannel(source,at));let nibbleAt=0,evmSum=0,evmCount=0,phaseSum=0,timingSum=0;
  for(let symbol=0;symbol<4;symbol++){if(!quiet)postMessage({type:'ofdm-symbol',symbol:symbol+1});const channelCodes=[];for(let ch=0;ch<2;ch++){const equalized=equalize(sources[ch],at+(symbol+1)*SYMBOL+CP,channels[ch]),re=equalized.re,im=equalized.im;const pilotBins=[24,37,51,65,79,93,107,119],expected=((index+symbol)&1)?-1:1,angles=[],magnitudes=[];for(const bin of pilotBins){angles.push(Math.atan2(im[bin]*expected,re[bin]*expected));magnitudes.push(Math.hypot(re[bin],im[bin]))}unwrap(angles);const line=fit(pilotBins,angles),gain=Math.max(median(magnitudes),1e-12);phaseSum+=line.intercept;timingSum+=-line.slope*FFT/(2*Math.PI);const codes=[];for(let carrier=0;carrier<CARRIERS;carrier++){if(PILOTS.has(carrier))continue;const bin=FIRST_BIN+carrier,phase=line.intercept+line.slope*bin,c=Math.cos(-phase),s=Math.sin(-phase),zr=(re[bin]*c-im[bin]*s)/gain,zi=(re[bin]*s+im[bin]*c)/gain,ri=nearest(zr),ii=nearest(zi);codes.push(ri|(ii<<1));evmSum+=(zr-LEVELS[ri])**2+(zi-LEVELS[ii])**2;evmCount++}channelCodes.push(codes)}for(let i=0;i<88;i++)nibbles[nibbleAt++]=channelCodes[0][i]|(channelCodes[1][i]<<2)}
  const scrambled=new Uint8Array(176);for(let i=0;i<176;i++)scrambled[i]=nibbles[i*2]|(nibbles[i*2+1]<<4);const raw=new DataView(scrambled.buffer),wireIndex=raw.getUint32(0,true)===MAGIC&&raw.getUint8(4)===3?raw.getUint16(22,true):index,packet=whiten(scrambled,wireIndex),view=new DataView(packet.buffer),sent=view.getUint32(28,true);packet.fill(0,28,32);const computed=crc32(packet);view.setUint32(28,sent,true);const evm=Math.sqrt(evmSum/Math.max(evmCount,1)),snr=-20*Math.log10(Math.max(evm,1e-9));return {packet,valid:view.getUint32(0,true)===MAGIC&&view.getUint8(4)===3&&sent===computed,wireIndex,sent,computed,score,evm,snr,phase:phaseSum/4,timing:timingSum/4,cfo:cpCfo(at),sampleOffset:at,swap}}
...
onmessage=event=>{const m=event.data;if(m.type==='connect-pcm'){pcmPort=event.ports?.[0]||null;if(pcmPort){pcmPort.onmessage=e=>receivePcm(e.data);pcmPort.start?.()}}else if(m.type==='arm'){armed=true;fecEpoch++;currentBlock=m.block||0;decoderMode='stereo';started=0;audioL=[];audioR=[];rsL=[];rsR=[];rsBase=0;rsPos=RS_HALF;preRoll=[];expectedPacket=m.index||0;crcOk=crcBad=drops=reportAt=receivedSamples=0;carrierReported=locked=false;lastPartial=0;lastPacketSeen=-1;groups.clear();failedGroups.clear();if(W){W.sxd_set_mode(0,0);W.sxd_reset(expectedPacket)}postMessage({type:'ofdm-armed',engine:W?'wasm':'js-fallback'})}else if(m.type==='boundary'){armed=false;audioL=[];audioR=[];rsL=[];rsR=[];carrierReported=locked=false;finalizeRecovery()}else if(m.type==='disarm'){armed=false;fecEpoch++;audioL=[];audioR=[];rsL=[];rsR=[];groups.clear();failedGroups.clear();carrierReported=locked=false}else if(m.type==='pcm')receivePcm(m)};
